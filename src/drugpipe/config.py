...
def _deep_merge(base: dict, override: dict) -> dict:
    ...
    将 `override` 递归合并到 `base`，会原地修改 `base`。
    """
...
def _apply_env_overrides(cfg: dict, prefix: str = "DP") -> dict:
    """Map environment variables with a prefix into nested config keys.
    将指定前缀的环境变量映射为嵌套配置字段。

    `DP_TARGET_TO_HIT__CHEMBL__PAGE_LIMIT=500`
    maps to `cfg["target_to_hit"]["chembl"]["page_limit"] = 500`.
    例如 `DP_TARGET_TO_HIT__CHEMBL__PAGE_LIMIT=500` 会映射到
...
def _auto_cast(val: str) -> Any:
    """Cast env string to bool/int/float/str when possible.
    尽可能将环境变量字符串转换为 bool/int/float/str。
    """
...
def load_config(
    ...
    """Load pipeline configuration from default/user/env/overrides.
    按默认配置、用户配置、环境变量和运行时覆盖项加载最终配置。
